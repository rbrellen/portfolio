...
```{r total-population, message = FALSE}
df %>% 
  ggplot(aes(x = reorder(state, total_population), y = total_population)) +
  geom_col(fill = 'dark blue') +
  geom_text(hjust = -0.1,
            size = 1.5,
            aes(label = scales::comma(total_population)),
            color = 'black') +
  coord_flip(ylim = c(0, 40000000)) +
  labs(title = 'States by Total Population',
       subtitle = 'Includes the District of Columbia and Puerto Rico',
       caption = 'Source: United States Census Bureau, 2010 Decennial Census',
       x = 'State',
       y = 'Total Population') +
  theme_classic() +
...
```{r top-ten-population, message = FALSE}
sorted_df <- df %>%
  as.data.frame() %>%
  dplyr::select(state,
                White = white,
...
```{r top-ten-percentage, message = FALSE}
sorted_df <- df %>%
  as.data.frame() %>%
  dplyr::select(state,
                `% White` = pct_white,
...
### Part 5: Static Mapping

#### Total population

```{r map-total-population, message = FALSE}
contiguous <- df %>% dplyr::filter(!state %in% c('Hawaii', 'Alaska', 'Puerto Rico'))

contiguous %>% 
  ggplot(aes(fill = total_population)) +
  geom_sf() +
  coord_sf(crs = 5070, datum = NA) +
  labs(title = 'Total Population by State',
       subtitle = 'Limited to 48 contiguous states and the District of Columbia',
       caption = 'Source: United States Census Bureau, 2010 Decennial Census') +
  theme_classic() +
...
```{r map-population-by-group, message = FALSE}
contiguous <- df %>% 
  dplyr::select(state,
                geoid,
                White = white,
  ...
  tidyr::gather(key = variable, value = value, -state, -geometry, -geoid)
...
contiguous %>%
  ...
  coord_sf(crs = 5070, datum = NA) +
  labs(title = 'Total Racial/Ethnic Group Population by State',
       subtitle = 'Limited to 48 contiguous states and the District of Columbia',
       caption = 'Source: United States Census Bureau, 2010 Decennial Census') +
  theme_classic() +
  theme(legend.position = 'bottom',
        plot.caption = element_text(hjust = 0, face = 'italic')) +
  scale_fill_viridis_c('Total Group Population',
                       option = 'magma',
                       direction = -1,
                       begin = 0.5,
                       end = 1,
...
#### Percentage of population by racial/ethnic group

```{r map-percent-population-by-group, message = FALSE}
contiguous <- df %>% 
  dplyr::select(state,
                geoid,
                `% White` = pct_white,
...
contiguous %>%
  ...
  facet_wrap(~variable, nrow = 2) +
  coord_sf(crs = 5070, datum = NA) +
  labs(title = 'Racial/Ethnic Group as Percent of State Population',
       subtitle = 'Limited to 48 contiguous states and the District of Columbia',
       caption = 'Source: United States Census Bureau, 2010 Decennial Census') +
  theme_classic() +
  theme(legend.position = 'bottom',
